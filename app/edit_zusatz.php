...
$fpath = $_SERVER['DOCUMENT_ROOT'] . $ne2_config_info['zusatzinfo_file'];
$oper = $_REQUEST['json_oper'];


$content_marker_startdivold   = '<div id="zusatzinfo" class="noprint">';
$content_marker_start =  $ne2_config_info['zusatzinfo_content_marker_start'];
$content_marker_startdiv =  $ne2_config_info['zusatzinfo_content_marker_startdiv'];
$content_marker_end =  $ne2_config_info['zusatzinfo_content_marker_end'] ;
...
if($oper == 'get_content') {  
	if ( strpos($fcontent, $content_marker_end) > 1) {
	      ...
	      // fallback mit RegExp: 
		$fcontent = preg_replace('/<\/div>\s*(<!-- end: zusatzinfo -->|)\s*$/','', $fcontent); 
	}
	...
	if($start_pos !== FALSE) {
		...
		if($start_pos !== FALSE) {
			...
			if($start_pos !== FALSE) {
				...
				$fcontent = preg_replace('/^\s*<div id="zusatzinfo"\s*>\s*/','', $fcontent);
			}
		}
	} 
	...
	echo('Die Zusatzinfo wurde aktualisiert');
}
?>
